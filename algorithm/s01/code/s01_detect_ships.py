...
def run(
    ...
    input_file=ROOT / "data/images",  # nitf file for detection
    ...
    save_img = True, # save results to *.tif
    output_format = 1, # 1: txt, 2: csv
):
    ...
    # 테스트 이미지를 1/div_num 만큼 width, height를 분할하고, 크롭된 이미지와 위치좌표를 반환
        for d_id, img0 in enumerate(div_img_list):
            ...
            pred = non_max_suppression(pred, conf_thres, iou_thres,max_det=max_det)#, classes
...
def parse_opt():
    ...
    parser.add_argument("--input_file", type=str, default=ROOT / "data/images", help="file of SLC data(nitf, tif, h5)")
    ...
    parser.add_argument('-O', "--output_dir", type=str, default="./output/", help="Path to update a single detection csv/txt file")
    parser.add_argument("--half", action="store_true", help="use FP16 half-precision inference")
    parser.add_argument("--save-img", action="store_true", help="save results to *.tif")
    parser.add_argument("--output_format", type=int, default=1, help="1: txt, 2: csv")
